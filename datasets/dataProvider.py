...
class DataProvider:
    ...
    def __init__(self, params: dict):
        ...
        self.train_dl = self._create_dataloader(train, shuffle=True, workers=workers, **params)
        self.val_dl = self._create_dataloader(val, shuffle=False, workers=workers, **params)
        self.test_dl = self._create_dataloader(test, shuffle=False, workers=workers, **params)

    @staticmethod
    def _create_dataloader(base_dataset, workers=4, batch_size=256, drop_last=False, shuffle=False, train_repeat=1,
                           collate_fn=None, **kwargs):
        if train_repeat > 1:
            base_dataset = ConcatDataset([base_dataset] * train_repeat)
...
class DatasetFactory:
    ...
    from datasets.cifar100 import CIFAR100, OrderedCIFAR100VAL
    from datasets.normal import NormalFullDataset

    dataset_params: dict
    all_datasets = [CIFAR10, CIFAR100, ImageNet100, OrderedCIFAR100VAL, NormalFullDataset]

    @staticmethod
    def build_dataset(params):
        dataset_name = params['dataset']
        dataset_params = params['dataset_params'] if 'dataset_params' in params else {}
...
if __name__ == "__main__":
    ...
    #     }
    # }
    # provider = DataProvider(hparams)
    # print([provider.train_dl.dataset[i][1] for i in range(100)])
    # print([provider.val_dl.dataset[i][1] for i in range(100)])
    # print([provider.test_dl.dataset[i][1] for i in range(100)])
    ...
    print([provider.val_dl.dataset[i][1] for i in range(100)])
    print([provider.test_dl.dataset[i][1] for i in range(100)])
    print(provider.train_dl.dataset[0][0].shape)
